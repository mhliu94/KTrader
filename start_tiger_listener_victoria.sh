...
usage() {
  cat <<'EOF'
Usage: ./start_tiger_listener.sh [options]
...
Options:
  ...
  --tiger-token-file PATH, --token-file PATH
      Optional Tiger OpenAPI token properties file. Only TBHK needs this.
...
Environment alternatives:
  ...
  TIGEROPEN_TOKEN_PATH or TIGER_TOKEN_FILE

Positional usage is also accepted:
  ./start_tiger_listener.sh tiger_openapi_config.properties [tiger_openapi_token.properties]
EOF
}
...
TIGER_TOKEN_PROPERTIES_PATH="${TIGEROPEN_TOKEN_PATH:-${TIGER_TOKEN_FILE:-}}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --tiger-props|--properties|--props|--config)
      ...
      shift 2
      ;;
    --tiger-token-file|--token-file|--token)
      [[ $# -ge 2 ]] || { echo "Missing value for $1" >&2; exit 2; }
      TIGER_TOKEN_PROPERTIES_PATH="$2"
      shift 2
      ;;
    -h|--help)
      usage
    ...
    *)
      ...
      elif [[ -z "$TIGER_TOKEN_PROPERTIES_PATH" ]]; then
        TIGER_TOKEN_PROPERTIES_PATH="$1"
      else
...
if [[ -n "$TIGER_PROPERTIES_PATH" ]]; then
  ...
  export_if_unset TIGER_TOKEN "$(read_property "$TIGER_PROPERTIES_FILE_RESOLVED" token || true)"
  export_if_unset TIGEROPEN_TOKEN "${TIGER_TOKEN:-}"
  ...
  if [[ -z "$TIGER_TOKEN_PROPERTIES_PATH" ]]; then
    local_token_file="$(dirname "$TIGER_PROPERTIES_FILE_RESOLVED")/tiger_openapi_token.properties"
    if [[ -f "$local_token_file" ]]; then
      TIGER_TOKEN_PROPERTIES_PATH="$local_token_file"
    fi
  fi
fi

if [[ -n "$TIGER_TOKEN_PROPERTIES_PATH" ]]; then
  TIGER_TOKEN_PROPERTIES_PATH="$(resolve_path "$TIGER_TOKEN_PROPERTIES_PATH")"
  if [[ ! -f "$TIGER_TOKEN_PROPERTIES_PATH" ]]; then
    echo "Tiger token properties file not found: $TIGER_TOKEN_PROPERTIES_PATH" >&2
    exit 2
  fi
  export TIGEROPEN_TOKEN_PATH="$TIGER_TOKEN_PROPERTIES_PATH"
  export_if_unset TIGER_TOKEN "$(read_property "$TIGER_TOKEN_PROPERTIES_PATH" token || true)"
  export_if_unset TIGEROPEN_TOKEN "${TIGER_TOKEN:-}"
fi
...
export SERVER_ID="${SERVER_ID:-tiger-listener-victoria}"

export TIGER_KAFKA_GROUP_ID="${TIGER_KAFKA_GROUP_ID:-tiger-trading-server-victoria}"
export TIGER_UI_ACCOUNT_IDS="${TIGER_UI_ACCOUNT_IDS:-ACC-TIGER-VICTORIA}"
export TIGER_UI_ACCOUNT_NUM_ID_MAP="${TIGER_UI_ACCOUNT_NUM_ID_MAP:-ACC-TIGER-VICTORIA:3}"
...
export TIGER_CASH_CURRENCIES="${TIGER_CASH_CURRENCIES:-USD,HKD}"
export TIGER_FOREX_SEG_TYPE="${TIGER_FOREX_SEG_TYPE:-SEC}"
if [[ -n "${TIGEROPEN_PROPS_PATH:-}" ]]; then
  export TIGER_TOKEN="${TIGER_TOKEN:-${TIGEROPEN_TOKEN:-}}"
  ...
  export TIGER_TOKEN="${TIGER_TOKEN:-${TIGEROPEN_TOKEN:-MTc3OTQxNDgzOTA5NiwxNzgyMDA2ODM5MDk25xBXCcTDWZa3W9tJM7Ochg==}}"
  export TIGER_ID="${TIGER_ID:-${TIGEROPEN_TIGER_ID:-20158804}}"
  export TIGER_ACCOUNT="${TIGER_ACCOUNT:-${TIGEROPEN_ACCOUNT:-62824418}}"
  export TIGER_SECRET_KEY="${TIGER_SECRET_KEY:-${TIGEROPEN_SECRET_KEY:-068672df-6560-36b5-843d-bb572ac97152}}"
fi
export TIGEROPEN_TOKEN="${TIGEROPEN_TOKEN:-$TIGER_TOKEN}"
...
export TIGER_LICENSE="${TIGER_LICENSE:-${TIGEROPEN_LICENSE:-}}"
export TIGEROPEN_LICENSE="${TIGEROPEN_LICENSE:-$TIGER_LICENSE}"

if [[ -z "${TIGER_PRIVATE_KEY_PATH:-}" && -z "${TIGER_PRIVATE_KEY:-}" && -f "$SCRIPT_DIR/tiger_private_key.pem" ]]; then
  export TIGER_PRIVATE_KEY_PATH="$SCRIPT_DIR/tiger_private_key.pem"
else
  export TIGER_PRIVATE_KEY_PATH="${TIGER_PRIVATE_KEY_PATH:-}"
fi
...
echo "  TIGEROPEN_TOKEN_PATH_SET=$([[ -n "${TIGEROPEN_TOKEN_PATH:-}" ]] && echo yes || echo no)"
echo "  TIGER_ID_SET=$([[ -n "$TIGER_ID" ]] && echo yes || echo no)"
echo "  TIGER_TOKEN_SET=$([[ -n "$TIGER_TOKEN" ]] && echo yes || echo no)"
